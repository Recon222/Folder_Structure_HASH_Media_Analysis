...
class BatchQueueWidget(QWidget):
    ...
    def __init__(self, parent=None, main_window=None):
        ...
        self.recovery_manager = self.controller.get_recovery_manager()
        self.processor_thread: Optional[BatchProcessorThread] = None
        self.processing_active = False
        
        # UI timer for statistics
    ...
    def _start_processing(self):
        ...
        # Use controller to start processing
        result = self.controller.start_batch_processing(self.main_window)
        if result.success:
            self.processor_thread = result.value
            ...
            error = UIError(
                str(result.error),
                user_message=result.error.user_message if hasattr(result.error, 'user_message') else str(result.error),
                component="BatchQueueWidget",
                severity=ErrorSeverity.ERROR
            )
            handle_error(error, {'operation': 'start_batch_processing'})
    ...
    def _cancel_processing(self):
        ...
        # Use controller to cancel processing
        result = self.controller.cancel_processing()
        if result.success:
            ...
            self.log_message.emit(f"Failed to cancel: {result.error}")
    ...
    def _on_batch_result_ready(self, result):
        ...
        # Tell controller processing is complete
        self.controller.complete_processing()
        self.processor_thread = None
    ...
    def cleanup(self):
        ...
        self.logger.info("Cleaning up BatchQueueWidget")
        ...
        try:
            ...
            # Delegate all resource cleanup to controller
            if self.controller:
